...
div.stButton>button{ 
    ...
    box-shadow:10px 10px 20px #d1d9e6, -10px -10px 20px #ffffff !important; font-weight:bold !important;
...
.stFileUploader, [data-testid="stFileUploader"] { 
    background: #F0F4F8 !important; border-radius: 20px !important;
    box-shadow: inset 8px 8px 16px #d1d9e6, inset -8px -8px 16px #ffffff !important;
    padding: 20px !important; border: 1px solid rgba(255,255,255,0.5) !important; 
}

/* 进度条动画 | Progress Animation */
.status-text{ color:#64B8FF !important; font-weight:600; text-align:left !important; margin:20px 0 28px 0; margin-left:0.25em; }
.stProgress > div > div > div > div { background: linear-gradient(to right, #64B8FF, #42F2BF) !important; }

/* 页脚 | Footer */
.footer { position: fixed; bottom: 0; left: 0; width: 100%; background: #F0F4F8; text-align:center; color:#BDC3C7; font-size:13px; padding-bottom:20px; z-index: 999; }
</style>
...
if uploaded_files:
    ...
    if st.button(" ", use_container_width=True, key="refine_btn"):
        ...
        # 完成后显示下载按钮 | Show download button when done
